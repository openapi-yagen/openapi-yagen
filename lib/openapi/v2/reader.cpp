...
SecuritySchemePtr parseSecurityScheme(const NodeWalker& w)
{
    ...
    if (type == "basic") {
        ...
        if (flowName == "implicit")
            ...
            logger.warn("<7b150a24> Unrecognized oauth2 \"flow\" value \"{}\" - security scheme ignored", flowName);
        s->flows = flows;
    } else {
        logger.warn("<e845ce78> Unrecognized security scheme type \"{}\" - kept as-is, unmapped", type);
        s->type = type;
    }
        ...
        if (in == "body") {
            ...
            if (!p->schema->type.empty() && p->schema->type.front() == "array") {
                ...
                if (cf == "csv") {
                    ...
                    logger.warn("<eb6c76dd> collectionFormat \"tsv\" has no OAS 3.x equivalent - "
