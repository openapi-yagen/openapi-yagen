...
Options:
  ...
  --header TEXT               Override the auto-generated "do not edit" header text prepended to every generated file (default: names the generator; wrapped in whichever comment syntax the file's own extension/generator.yml commentStyle resolves to)
  --no-header                 Disable the auto-generated "do not edit" header entirely
```

#### Auto-generated file headers

Every file written via `renderTemplate`/`copyFile` (see [`generator-format.md`](docs/generator-format.md))
gets a "do not edit" header prepended automatically, wrapped in whatever comment syntax that
file's own extension uses (`#` for `.rb`/`.py`/`.yml`, `//` for `.kt`/`.go`/`.ts`/`.h`/..., and so
on - see [`lib/generator/file_header.cpp`](lib/generator/file_header.cpp) for the full table). An
extension the table doesn't recognize (e.g. a hypothetical `.json` output, which has no comment
syntax at all) is left untouched rather than corrupted with a guess. `renderTemplateToString`'s
output never gets one, since it's an in-memory fragment (e.g. for a `{% include %}`-style
partial), not a real output file.

The default header names the specific generator that produced the file:

```
// Code generated by openapi-yagen (kotlin_ktor_server). DO NOT EDIT.
// https://openapi-yagen.github.io
```

`--header "<text>"` replaces the message itself (still auto-wrapped in the resolved comment
syntax); `--no-header` disables the header entirely for that run. A generator can also override
which comment style the engine picks for *its own* output, regardless of extension, via
`generator.yml`'s `commentStyle` field (see [`generator-format.md`](docs/generator-format.md)) -
useful if the built-in extension table guesses wrong for an unusual output format.
