...
TEST_CASE("copyFile copies a static file verbatim without templating", "[generator]")
{
    ...
    // The engine's own auto-generated-file header (see file_header.h) is prepended ahead of the
    // verbatim-copied content - checked separately from the "copyFile doesn't template" content
    // assertion below, which still needs the copied bytes to appear completely untouched.
    REQUIRE_THAT(fileWriter->files["out/Validation.kt"],
                 Catch::Matchers::StartsWith(
                     "// Code generated by openapi-yagen (test_generator). DO NOT EDIT.\n"
...
TEST_CASE("--no-header (opts.noHeader) suppresses the auto-generated header entirely", "[generator]")
{
    auto fileWriter = make_shared<MockedFileWriter>();
    auto templateRenderer = make_shared<MockedTemplateRenderer>();
    ...
    MockedFileReaderBackend::Files files = {
        { "main.js", "copyFile(\"Validation.kt\", \"out/Validation.kt\")" },
        { "generator.yml", readResource("generator.yml") },
        { "Validation.kt", "fun requireMin(v: Int, min: Int, label: String) { }\n" },
    };
    ...
    REQUIRE(fileWriter->files["out/Validation.kt"] == "fun requireMin(v: Int, min: Int, label: String) { }\n");
}

TEST_CASE("--header (opts.headerText) overrides the default header text", "[generator]")
{
    auto fileWriter = make_shared<MockedFileWriter>();
    auto templateRenderer = make_shared<MockedTemplateRenderer>();
    ...
    REQUIRE(fileWriter->files["out/Validation.kt"] == "// Custom header text\n\nfun requireMin(v: Int, min: Int, label: String) { }\n");
}

TEST_CASE("An unrecognized output file extension skips header injection entirely", "[generator]")
{
    auto fileWriter = make_shared<MockedFileWriter>();
    auto templateRenderer = make_shared<MockedTemplateRenderer>();

    MockedFileReaderBackend::Files files = {
        { "main.js", "copyFile(\"data.bin\", \"out/data.json\")" },
        { "generator.yml", readResource("generator.yml") },
        { "data.bin", "{\"a\":1}" },
    };
    ...
    // .json has no comment syntax at all (see file_header.cpp's table) - a header there would
    // corrupt the file, so it's skipped rather than guessed at.
    REQUIRE(fileWriter->files["out/data.json"] == "{\"a\":1}");
}

TEST_CASE("generator.yml's commentStyle overrides the built-in extension table", "[generator]")
{
    auto fileWriter = make_shared<MockedFileWriter>();
    auto templateRenderer = make_shared<MockedTemplateRenderer>();
    ...
    MockedFileReaderBackend::Files files = {
        { "main.js", "copyFile(\"Validation.kt\", \"out/Validation.kt\")" },
        { "generator.yml", "name: hash_style_generator\ncommentStyle: \"#\"\nmainScriptPath: main.js\n" },
        { "Validation.kt", "fun requireMin(v: Int, min: Int, label: String) { }\n" },
    };
    ...
    // Despite .kt normally mapping to "//" (see file_header.cpp), the generator's own declared
    // commentStyle wins.
    REQUIRE_THAT(fileWriter->files["out/Validation.kt"],
                 Catch::Matchers::StartsWith("# Code generated by openapi-yagen (hash_style_generator). DO NOT EDIT.\n"
                                             "# https://openapi-yagen.github.io\n\n"));
}

TEST_CASE("renderTemplateToString never writes a file, so it never gets an auto-generated header", "[generator]")
{
    auto fileWriter = make_shared<MockedFileWriter>();
    auto templateRenderer = make_shared<MockedTemplateRenderer>();

    MockedFileReaderBackend::Files files = {
        { "main.js", "renderTemplateToString(\"inner_template\", {}, {})" },
        { "generator.yml", readResource("generator.yml") },
    };
    ...
    // renderTemplateToString never reaches fileWriter->write at all (it hands the string straight
    // back to JS) - so writeGeneratedFile() never runs for it, and nothing gets written here.
    REQUIRE(fileWriter->files.empty());
}
