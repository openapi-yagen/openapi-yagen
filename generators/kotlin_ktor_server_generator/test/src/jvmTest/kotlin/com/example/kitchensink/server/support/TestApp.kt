...
// section (ContentNegotiation + StatusPages mapping BadRequestException/NumberFormatException to
// 400), plus one addition this test suite introduces itself: mapping a test-local
// NotFoundException to 404. That mapping is NOT a generator feature - "not found" is business
// logic the generator has no opinion about (see the README's "Known limitations"); a real
// integrator is expected to define and map their own such exception exactly like this.
    ...
    application {
        ...
        install(StatusPages) {
            ...
            exception<NumberFormatException> { call, _ ->
                call.respondText("Invalid parameter", status = HttpStatusCode.BadRequest)
            }
