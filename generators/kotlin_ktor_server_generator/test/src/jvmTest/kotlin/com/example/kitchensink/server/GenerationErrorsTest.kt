...
// Shells out to the openapi-yagen binary directly (not through Gradle's own generateServer task,
...
class GenerationErrorsTest {
    ...
    fun `an unsupported request body content-type aborts generation by default (strict=true)`() {
        val outDir = Files.createTempDirectory("kotlin-server-unsupported-strict-").toFile()
        try {
    ...
    fun `an unsupported request body content-type is skipped with a warning under strict=false`() {
        val outDir = Files.createTempDirectory("kotlin-server-unsupported-permissive-").toFile()
        try {
            ...
            assertTrue(output.contains("text/plain"), output)
            assertFalse(File(outDir, "apis/NotesApiHandler.kt").exists())
        } finally {
            outDir.deleteRecursively()
        }
    }
