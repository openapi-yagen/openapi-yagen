...
function registerUnion(registry, name, schema, variantOpts) {
  ...
  try {
    ...
    // than guessing, fall back to the same "any"-typed escape hatch already used for genuinely
    // free-form schemas (see primitiveGoType's caller below) and let the caller sort the real
    ...
    addModel(registry, name, { name, kind: "alias", targetType: "any", description: schema.description || null });
    return;
  }
