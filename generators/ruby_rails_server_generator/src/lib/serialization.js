...
// (a generated model instance, recursively for nested arrays/hashes/models). Forked from
// ruby_faraday_client_generator's lib/serialization.js - unchanged except for the "date"/
// "datetime" descriptor kind (see lib/types.js) and format:uuid validation, both worth doing here
// but not for a client only ever constructing outgoing values (see that generator's README).
//
...
//   { kind: "date" }                      // format: date - parsed into a Ruby Date
//   { kind: "datetime" }                  // format: date-time - parsed into a Ruby Time
...
export function buildValidateStatements(descriptor, schema, expr, fieldLabel, depth = 0) {
  ...
  //
  // "date"/"datetime" get a basic-type check against Ruby's own Date/Time - unlike from_h (which
...
export function buildToWireExpr(descriptor, expr, depth = 0) {
  switch (descriptor.kind) {
    ...
    case "date":
    case "datetime":
      // Date#iso8601/Time#iso8601 (both stdlib, required by runtime.rb) round-trip back to exactly
      // the ISO 8601 wire form parse_date/parse_datetime above accept.
      return `${expr}&.iso8601`;
    case "array": {
      const v = `item${depth}`;
