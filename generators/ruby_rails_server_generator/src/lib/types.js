// Maps OpenAPI schemas to a registry of named Ruby models to render (see main.js). Forked from
// ruby_faraday_client_generator's lib/types.js (models are entirely schema-driven and
// transport-agnostic, so almost all of that generator's logic applies unchanged here) - the only
// difference is that `format: date`/`date-time` get their own descriptor kinds ("date"/"datetime",
// mapped to Ruby's Date/Time) instead of staying a plain String: a server *parses untrusted wire
// input*, so parsing-as-validation (see lib/serialization.js's buildFromHExpr) is worth doing here
// in a way it isn't for a client only ever *constructing* a request from values the caller's own
// code already produced (see ruby_faraday_client_generator's README "Known limitations" - this is
// the same "only server generators in this project validate format:X" position taken for
// format:uuid there, extended to format:date/date-time too).
//
...
// parameter's default value - `from_h` uses the very same literal (via a `h.key?(...)` ternary in
// registerClass below) so both entry points (direct construction and JSON deserialization) apply
// it identically. Returns null for any shape not recognized (e.g. an object/array default, a
// date/date-time default, or an enum value that doesn't match one of its own entries) - the
// property then falls back to its ordinary `nil` default, same as if `default` were absent.
...
// Forked from ruby_faraday_client_generator's lib/types.js: `format: date`/`date-time` get their
// own descriptor kind here (mapped to Ruby's Date/Time - see lib/serialization.js) instead of
...
function primitiveDescriptor(s) {
  ...
  if (s.type === "string" && s.format === "date-time") return { kind: "datetime" };
  const type = s.type;
...
function primitiveLabel(s) {
  ...
  if (s.type === "string" && s.format === "date-time") return "Time";
...
export function buildModelRegistry(root) {
  ...
  for (const rawName of Object.keys(schemas)) {
    ...
    if (existing) {
      ...
      throw Error(`<d68e061b> Schema name collision after Ruby identifier conversion: "${rawName}" -> "${name}"`);
    }
    withResilience(
      `schema "${rawName}"`,
