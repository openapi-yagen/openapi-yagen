...
class GenerateModeTest {
    ...
    fun `an unrecognized generate value fails generation with the allowed values listed`() {
        val outDir = Files.createTempDirectory("kotlin-generatemode-invalid-").toFile()
        try {
    ...
    @Test
    fun `generate=api skips models but still generates the API classes`() {
        val outDir = Files.createTempDirectory("kotlin-generatemode-api-").toFile()
        try {
            ...
            assertTrue(File(outDir, "QueryUtils.kt").exists())
            assertTrue(File(outDir, "apis/PetsApi.kt").exists())
        } finally {
            outDir.deleteRecursively()
        }
    }
    ...
    @Test
    fun `generate=models generates only the models`() {
        val outDir = Files.createTempDirectory("kotlin-generatemode-models-").toFile()
        try {
            val (exitCode, output) = generate(outDir, "-v", "generate=models")
            assertTrue(exitCode == 0, "expected generation to succeed: $output")
            assertTrue(File(outDir, "models/Pet.kt").exists())
            assertFalse(File(outDir, "ApiClient.kt").exists())
            assertFalse(File(outDir, "QueryUtils.kt").exists())
